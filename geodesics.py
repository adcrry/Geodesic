...
D_f = 5 #[-D_f,D_f]^2 is a square domain center at 0 where f is defined
...
X_0 = normalize_speed(X_0) #normalise the initial velocity vector
...
geodesic = np.array([lambdify([x,y], f[0])(sol.y[0],sol.y[1]),  lambdify([x,y], f[1])(sol.y[0],sol.y[1]), lambdify([x,y], f[2])(sol.y[0],sol.y[1])])
...
X = np.linspace(-D_f, D_f, 30)
Y = np.linspace(-D_f, D_f, 30)
